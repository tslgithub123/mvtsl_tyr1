...
const OneUsers = () => {
    ...
    const exportToExcel = () => {
        ...
        const ws = XLSX.utils.aoa_to_sheet([
            ...
            ["", "", "MV1.0", "", ...oneUsers.map(user => user.question_19)],
            ["", "", "MV2.0", "", ...oneUsers.map(user => user.question_24)],
            ["", "", "MV3.0", "", ...oneUsers.map(user => user.question_29)],

            ["75", "1.1.2 Yes/No", "Tree Census with Geotagging –Report Prepared and Published during Majhi Vasundhara Abhiyan 4.0", "", ...oneUsers.map(user => user.question_34)],
            ...
            ["", "", "MV1.0", "", ...oneUsers.map(user => user.question_71)],
            ["", "", "MV2.0", "", ...oneUsers.map(user => user.question_77)],
            ["", "", "MV3.0", "", ...oneUsers.map(user => user.question_83)],
            ...
            ["80", "1.1.6 Yes/No", "Usage of compost- % of Compost sold/ self utilized", "", ...oneUsers.map(user => user.question_119)],

            ["100", "1.2.1", "Percentage of solid waste segregated at source and collected", ""],
            ["50", " 1(a)  ", "Segregation at source", "", ...oneUsers.map(user => user.question_126)],
            ["50", "1(b) ", "Collection", ...oneUsers.map(user => user.question_132)],
        ...
        XLSX.writeFile(wb, "5k-10k_Population_Cumulative.xlsx");
    };
    ...
    const fetchData = async (page) => {
        try {
            ...
            let endpoint;
            if (isAdmin) {
                endpoint = 'http://89.116.20.162:8080/api/v1/cities/rural/category/5k-10k';
            } else {
    ...
    return (
        <Layout>
            <div className='addUsertt'>
                <h1 className="text-center m-2">Cumulative of 5K-10K Population</h1>
