...
const OneUsers = () => {
    ...
    const exportToExcel = () => {
        ...
        const ws = XLSX.utils.aoa_to_sheet([
            ...
            ["(Relative Marking)", "3.7", "Number of promotional activities for eco-friendly immersion ", "20", ...oneUsers.map(user => user.question_533)],
            ["(Relative Marking)", "3.7", "No. of artificial immersion spots created", "50", ...oneUsers.map(user => user.question_539)],
            ["(Relative Marking)", "3.7", "Collection, segregation, transport and processing of worship material pre and post immersion", "30", ...oneUsers.map(user => user.question_545)],


            ["(Relative Marking)", "3.8", "No. of promotional activity done ", "50", ...oneUsers.map(user => user.question_549)],
            ["(Relative Marking)", "3.8", "Percentage of eco-friendly idols  worshipped Community", "50", ...oneUsers.map(user => user.question_555)],
            ["(Relative Marking)", "3.8", "Percentage of eco-friendly idols worshipped Individual", "50", ...oneUsers.map(user => user.question_561)],
        ...
        XLSX.writeFile(wb, "2.5k-5k_Population_Cumulative.xlsx");
    };
    ...
    const fetchData = async (page) => {
        try {
            ...
            let endpoint;
            if (isAdmin) {
                endpoint = '/api/v1/cities/rural/category/2.5k-5k';
            } else {
    ...
    return (
        <Layout>
            <div className='addUsertt'>
                <h1 className="text-center m-2">Cumulative of 2.5K-5K Population</h1>
